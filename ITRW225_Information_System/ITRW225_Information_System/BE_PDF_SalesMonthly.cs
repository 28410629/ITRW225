    ...
    public class BE_PDF_SalesMonthly
    {
        public string createPDF( string chartPath, List<string[]> listSales)
        {
            try
            {
                ...
                string year = listSales[0][0].Remove(4);
                string month = listSales[0][0].Substring(5);
                month = month.Remove(2);

                string reportDay = year + "-" + month;
                string filename = appRootDir + "\\Sales Report Month for " + reportDay + " created on " + day + ".pdf";
                ...
                {
                    doc.AddTitle("Sales Report Month - " + day);
                    ...
                    Paragraph client = new Paragraph(@"MONTH SALES REPORT FOR " + reportDay + " CREATED ON " + DateTime.Today.Year + "-" + DateTime.Today.Month + "-" + DateTime.Today.Day + " AT " + DateTime.Now.ToString("h:mm tt"));
                    ...
                    if (listSales[0][1] != "")
                    {
                        ...
                        {
                            price = Convert.ToDouble(listSales[i][2]);
                            priceTotal += price;

                            cell = new PdfPCell(new Phrase(listSales[i][0]));
                            cell.HorizontalAlignment = Element.ALIGN_CENTER;
                            table.AddCell(cell);
                            ...
                            cell = new PdfPCell(new Phrase(listSales[i][2]));
                            cell.HorizontalAlignment = Element.ALIGN_CENTER;
                            table.AddCell(cell);
                        }
